...
class MixEHR_Seed(nn.Module):
    ...
    def initialize_tokens(self):
        ...
        self.exp_n[0] = torch.load(os.path.join(self.init_priors, "init_exp_n_icd.pt"),
                                   map_location=device)  # get exp_n for guided ICD modality, V X K matrix
        self.exp_n[1] = torch.load(os.path.join(self.init_priors, "init_exp_n_med.pt"),
                                   map_location=device)  # get exp_n for unguided med modaltiy, V X K matrix
        self.exp_n[2] = torch.load(os.path.join(self.init_priors, "init_exp_n_cpt.pt"),
                                   map_location=device)  # get exp_n for unguided med modaltiy, V X K matrix
        self.exp_n[3] = torch.load(os.path.join(self.init_priors, "init_exp_n_drg.pt"),
                                   map_location=device)  # get exp_n for unguided med modaltiy, V X K matrix
        self.exp_n[4] = torch.load(os.path.join(self.init_priors, "init_exp_n_lab.pt"),
                                   map_location=device)  # get exp_n for unguided med modaltiy, V X K matrix
        self.exp_n[5] = torch.load(os.path.join(self.init_priors, "init_exp_n_note.pt"),
                                   map_location=device)  # get exp_n for unguided med modaltiy, V X K matrix
    ...
    def get_elbo(self, batch_indices, batch_C, minibatch, epoch, start_time):
        ...
        constant_terms = self.D * self.K * torch.lgamma(torch.tensor(self.eta)) - self.D * torch.lgamma(torch.tensor(self.K * self.eta))
        ...
        print("elbo: ", loss.detach().cpu().numpy().item(), " p_z: ", torch.sum(p_z), " q_z: ", self.exp_q_z, " p_w: ", logsumexp([log_sum_n_terms, log_sum_s_terms]))
    ...
    def SCVB0_guided(self, batch_BOW, batch_indices, batch_C, iter_n, guided_m=0):
        ...
        for d_i, doc_id in enumerate(batch_indices):
            ...
            # print(self.seeds_topic_matrix.shape)
            # print(BOW_nonzero)
        ...
        if self.stochastic_VI:
            ...
            print("temp_exp_s: ", temp_exp_s.shape)
            print("Guided V: ", self.V[guided_m])
            print("exp_s: ", self.exp_s.shape)
    ...
    def SCVB0_unguided(self, batch_BOW, batch_indices, batch_C, iter_n, unguided_m):
        ...
        for d_i, doc_id in enumerate(batch_indices):
            ...
            # if unguided_m == 3:
            #     print(temp_gamma.shape)
            #     # Find the index of the maximum value in each row
            #     argmax_indices = temp_gamma.argmax(axis=1)
            #     argmax_indices = argmax_indices.type(torch.int16)
            #     print(argmax_indices.shape)  # Array of indices of the maximum values
            #     print(argmax_indices.dtype)  # Array of the maximum values in each row
            # calculate sufficient statistics
